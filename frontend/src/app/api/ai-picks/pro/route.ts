...
async function findCSVFile(filename: string): Promise<string> {
  ...
  for (const p of possiblePaths) {
    try {
      await fs.access(p);
      console.log(`[AI Picks Pro] Found CSV at: ${p}`);
      return p;
    } catch {
  ...
  console.error(`[AI Picks Pro] Could not find ${filename} in any of:`, possiblePaths);
...
export async function GET() {
  try {
    ...
    const csvPath = await findCSVFile('top25_pro.csv');
